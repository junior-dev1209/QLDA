...
async function offlineCredentialDigest(password, salt) {
  ...
  const bits = await window.crypto.subtle.deriveBits(
    { name: "PBKDF2", salt, iterations: OFFLINE_LOGIN_PBKDF2_ITERATIONS, hash: "SHA-256" },
    material,
    256,
  );
...
async function verifyOfflineLogin(username, password) {
  ...
  if (!proof || !password || Number.isNaN(new Date(proof.expiresAt || "").getTime()) || new Date(proof.expiresAt).getTime() < Date.now()) return null;
  const account = (state.accounts || []).find(
    (item) => String(item?.id || "") === String(proof.accountId || "") && normalizedLoginUsername(item?.username) === normalizedUsername,
  );
  if (!account || account.disabled) return null;
  try {
    const verifier = await offlineCredentialDigest(password, base64ToBytes(proof.salt));
    return verifier && secureTextEquals(verifier, proof.verifier) ? account : null;
  } catch {
    return null;
  }
}
...
function scheduleSharedStateRetry() {
  if (!sharedSync.session || !sharedSync.dirty || sharedSync.retryTimer) return;
...
function scheduleSharedStateRefresh({ immediate = false } = {}) {
  if (sharedSync.refreshTimer || !sharedSync.session) return;
...
async function refreshSharedState() {
  if (!sharedSync.session || sharedSync.inFlight || document.visibilityState === "hidden") return;
...
function canOpenTask(task) {
  return canEditTaskDetails(task) || canDeleteTask(task) || canReportTask(task) || canCollaborateTask(task) || canReviewTaskCompletion(task);
}
...
function reviewTaskCompletion(taskId, decision, note = "", qualityPercent = "") {
  ...
  logActivity({
    ...
    detail: `${auditedTask.title} · ${previousStatus} -> ${nextStatus}${approved && completionTimingLabel ? ` · ${completionTimingLabel}` : ""}${approved ? ` · chất lượng ${formatScore(assessedQuality)}%` : ""}${note ? ` · ${String(note).trim()}` : ""}`,
  });
  saveState();
  renderAll();
  return true;
      ...
      (report, index) => {
        ...
        return `
          <div class="progress-report-row">
            ...
            ${isCompletionReview ? `<span><strong>Đánh giá hoàn thành:</strong> ${reviewResult}</span>` : `<span><strong>${escapeHtml(transition)}</strong> · Tiến độ ${formatScore(report.progress)}%</span>`}
